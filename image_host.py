...
支持 sm.ms 免费图床（国内可访问，无需注册）
备用: 如果 sm.ms 不可用，回退到本地保存
"""
import requests
from pathlib import Path


def upload_to_smms(filepath: str, retries: int = 2) -> str | None:
    """
    上传图片到 sm.ms 图床
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if data.get("code") == "success":
                ...
                # 图片已存在，返回已有URL
                url = data["images"]
                print(f"  [图床] ✅ 使用已有图片: {url}")
                return url
            else:
                msg = data.get("msg", "未知错误")
                print(f"  [图床] ⚠️ 上传失败 (尝试 {attempt+1}/{retries+1}): {msg}")
        except requests.exceptions.Timeout:
            print(f"  [图床] ⚠️ 超时 (尝试 {attempt+1}/{retries+1})")
        except Exception as e:
            print(f"  [图床] ⚠️ 错误: {e} (尝试 {attempt+1}/{retries+1})")
